...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("Error fetching PU reports:", error)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data: any = {
      ...
      keteranganTL: status === "TL" ? (keterangan || "") : null,
      keteranganBTL: status === "BTL" ? (keterangan || "") : null,
      status: status || "BTL",
    }
    ...
    console.error("Error saving PU report:", error)
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    console.error("Error deleting PU report:", error)
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 })
  }
}
