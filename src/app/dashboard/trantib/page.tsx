...
const defaultTrantibSeeds = [
  { id: 'tr-1', tanggal: '2026-04-09', posyandu: 'Segar', nik: '1807041906890006', nama: 'Juni Prayogi', alamat: 'Dusun 4', hal: 'Aktifkan Ronda Malam', keteranganTL: 'Susun Jadwal Ronda', keteranganBTL: '', status: 'TL' },
  { id: 'tr-2', tanggal: '2026-04-11', posyandu: 'Utama', nik: '1807041707750005', nama: 'Solehan', alamat: 'Dusun 1', hal: 'Aktifkan Ronda Malam', keteranganTL: 'Susun Jadwal Ronda', keteranganBTL: '', status: 'TL' },
  { id: 'tr-3', tanggal: '2026-04-13', posyandu: 'Mulia', nik: '3213031811860001', nama: 'Hani Handoko', alamat: 'Dusun 2', hal: 'Aktifkan Ronda Malam', keteranganTL: 'Susun Jadwal Ronda', keteranganBTL: '', status: 'TL' },
  { id: 'tr-4', tanggal: '2026-04-15', posyandu: 'Giat', nik: '1807042411920001', nama: 'Candra wiranata', alamat: 'Dusun 3', hal: 'Aktifkan Ronda Malam', keteranganTL: 'Susun Jadwal Ronda', keteranganBTL: '', status: 'TL' },
  { id: 'tr-5', tanggal: '2026-04-17', posyandu: 'Lestari', nik: '1807040605750004', nama: 'Sarno Haryanto', alamat: 'Dusun 5', hal: 'Aktifkan Ronda Malam', keteranganTL: 'Susun Jadwal Ronda', keteranganBTL: '', status: 'TL' },
  { id: 'tr-6', tanggal: '2026-06-09', posyandu: 'Segar', nik: '1807044408790002', nama: 'Fitriyah', alamat: 'Dusun 4', hal: 'Pengamanan Tempat Hajatan', keteranganTL: 'Segera ditindaklanjuti', keteranganBTL: '', status: 'TL' }
]
...
export default function TrantibPage() {
  ...
  const fetchReports = async (pId: string, yr: number) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/dashboard/trantib?posyanduId=${pId}&tahun=${yr}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const res = await fetch(`/api/dashboard/trantib?id=${id}`, {
          method: 'DELETE'
        })
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/dashboard/trantib', {
        method: 'POST',
  ...
  const handleExportExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet('TRANTIB')
    ...
    // Row 1: LAPORAN POSYANDU
    worksheet.mergeCells('A1:I1')
    const r1 = worksheet.getCell('A1')
    r1.value = 'LAPORAN POSYANDU'
    ...
    // Row 2: BIDANG TRANTIBUM LINMAS
    worksheet.mergeCells('A2:I2')
    const r2 = worksheet.getCell('A2')
    r2.value = 'BIDANG TRANTIBUM LINMAS'
    r2.font = { name: 'Arial', size: 12, bold: true, color: { argb: 'FFFFFFFF' } }
    r2.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E3A8A' } } as any
    r2.alignment = { vertical: 'middle', horizontal: 'center' }
    worksheet.getRow(2).height = 30

    // Row 3: DESA : TULUSREJO
    worksheet.mergeCells('A3:I3')
    const r3 = worksheet.getCell('A3')
    r3.value = `DESA : ${namaDesa.toUpperCase()}`
    ...
    // Row 5-6: Header bertingkat
    worksheet.getRow(5).values = ['No', 'Tanggal', 'Posyandu', 'NIK', 'Nama', 'Alamat', 'Hal Pengaduan', 'Keterangan', '']
    worksheet.getRow(6).values = ['', '', '', '', '', '', '', 'TL', 'BTL']
    ...
    filtered.forEach((r, idx) => {
      ...
      row.values = [
        ...
        r.alamat,
        r.hal,
        r.status === 'TL' ? (r.keteranganTL || 'Sudah ditindaklanjuti') : '',
        r.status === 'BTL' ? (r.keteranganBTL || 'BTL') : ''
      ]
      row.height = 20
    ...
    const totalRowIdx = 8 + filtered.length
    ...
    link.download = `TRANTIB_${namaDesa.toUpperCase()}.xlsx`
    link.click()
  }
  ...
  const handleExportCSV = () => {
    ...
    const rows = filtered.map((r, idx) => {
      return [
        ...
        `"${(r.alamat || '').replace(/"/g, '""')}"`,
        `"${r.hal.replace(/"/g, '""')}"`,
        `"${(r.status === 'TL' ? (r.keteranganTL || 'Sudah ditindaklanjuti') : '').replace(/"/g, '""')}"`,
        `"${(r.status === 'BTL' ? (r.keteranganBTL || 'BTL') : '').replace(/"/g, '""')}"`
      ].join(',')
    })
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = `TRANTIB_${namaDesa.toUpperCase()}.csv`
    link.click()
  }
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={report.id} className="border-b border-slate-200/70 dark:border-white/10 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {report.status === 'TL' ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300">
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                ...
                <div className="md:col-span-2 border-t border-slate-200/70 dark:border-white/10 pt-4">
                  <DataDukungUpload files={uploadedFiles} setFiles={setUploadedFiles} bidang="TRANTIB" posyanduId={selectedPosyanduId} />
                </div>
