...
import { Plus, Search, Filter, HardHat, FileText, MapPin, Clock, CheckCircle2, XCircle, ArrowLeft, Download, Edit2, Trash2, X, Eye } from 'lucide-react'
...
export default function PekerjaanUmumPage() {
  ...
  const dummyReports = [
    {
      ...
      tanggal: '2025-06-10',
      nama: 'Ahmad Sudirman',
      nik: '1807011002930001',
      alamat: 'RT 02 / RW 01',
      keluhan: 'Mengajukan pembuatan sumur bor karena di musim kemarau kekeringan.',
      lokasi: 'Dusun 2, Dekat Masjid Al-Ikhlas',
      status: 'BTL',
  ...
  const initialForm = { tanggal: '', nama: '', nik: '', alamat: '', keluhan: '', lokasi: '', status: 'BTL', noSuratRT: '' }
  ...
  const handleEdit = (report: any) => {
    ...
    setEditId(report.no.toString())
    setIsModalOpen(true)
  }

  const handleDelete = (no: number) => {
    if(confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      setReports(reports.filter(r => r.no !== no))
    }
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (editId) {
      setReports(reports.map(r => r.no.toString() === editId ? ({ ...r, ...formData, image: imagePreview, pdf: pdfPreview } as any) : r))
    } else {
      setReports([{ ...formData, no: Date.now(), keterangan: 'Baru', scope: 'POSYANDU', image: imagePreview, pdf: pdfPreview } as any, ...reports])
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        <div>
          ...
          <p className="text-slate-500 dark:text-zinc-400 text-sm">Sistem Informasi Posyandu (Pekerjaan Umum)</p>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 dark:text-zinc-400 text-sm">
                {isPosyandu ? 'Pengaduan & Aspirasi Bidang Pekerjaan Umum' : `Pengaduan & Aspirasi Bidang Pekerjaan Umum - ${selectedPosyandu}`}
              </p>
            </div>
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-xl border border-slate-100 dark:border-zinc-700 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs uppercase text-slate-500 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-700/50">
                  <tr>
                    <th className="px-4 py-3">No</th>
                    <th className="px-4 py-3">Tanggal</th>
                    <th className="px-4 py-3">No Surat RT</th>
                ...
                <tbody>
                  {reports.filter(r => r.nama.toLowerCase().includes(search.toLowerCase()) || r.keluhan.toLowerCase().includes(search.toLowerCase())).map((report) => (
                    <tr key={report.no} className="border-b border-slate-100 dark:border-zinc-700 hover:bg-slate-50/50 dark:hover:bg-zinc-700/20 transition-colors">
                      <td className="px-4 py-3 text-slate-500 dark:text-zinc-400">{report.no}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-white font-medium">{report.tanggal}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-slate-800 dark:text-white">{report.nama}</p>
                            <p className="text-xs text-slate-500 dark:text-zinc-400">{report.nik}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{report.alamat}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300 max-w-xs truncate">{report.keluhan}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{report.lokasi}</td>
                    <td className="px-4 py-3">
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {(isPosyandu || role === 'SUPERADMIN') && (
                          <>
                            ...
                            <button onClick={() => handleDelete(report.no)} className="text-rose-500 hover:text-rose-600 transition-colors" title="Hapus">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                ...
                <div className="md:col-span-1">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-200 block mb-1.5">NIK Pemohon</label>
                  <input
                    type="text"
                    value={formData.nik}
                ...
                <div className="md:col-span-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-200 block mb-1.5">Keluhan / Permohonan</label>
                  <textarea
                ...
                <div className="md:col-span-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-200 block mb-1.5">Lokasi Infrastruktur</label>
                  <input
                    type="text"
                    value={formData.lokasi}
                    onChange={(e) => setFormData({...formData, lokasi: e.target.value})}
                    className="block w-full bg-slate-50 dark:bg-zinc-800 border border-transparent focus:border-emerald-500 dark:focus:border-emerald-500 rounded-xl px-4 py-2.5 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all"
                    placeholder="Dusun 2, Dekat Masjid..."
                    required
                  />
                </div>
            ...
            >
              ...
              <div className="p-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 dark:text-zinc-400 uppercase font-medium">No Surat RT</p>
                    <p className="text-sm text-slate-800 dark:text-white font-medium mt-0.5">{selectedReportForView.noSuratRT || '-'}</p>
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    <p className="text-xs text-slate-500 dark:text-zinc-400 uppercase font-medium">Keluhan</p>
                    <p className="text-sm text-slate-800 dark:text-white mt-0.5 bg-slate-50 dark:bg-zinc-800 p-3 rounded-lg border border-slate-100 dark:border-zinc-700">{selectedReportForView.keluhan}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 dark:text-zinc-400 uppercase font-medium">Lokasi</p>
                    <p className="text-sm text-slate-800 dark:text-white font-medium mt-0.5">{selectedReportForView.lokasi}</p>
                  </div>
                  <div>
