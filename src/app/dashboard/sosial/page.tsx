...
export default function SosialPage() {
  ...
  const fetchReports = async (pId: string, yr: number) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/dashboard/sosial?posyanduId=${pId}&tahun=${yr}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const res = await fetch(`/api/dashboard/sosial?id=${id}`, {
          method: 'DELETE'
        })
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/dashboard/sosial', {
        method: 'POST',
              ...
              <table className="w-full text-sm text-left text-[var(--dash-text-soft)]">
                <thead className="text-[11px] uppercase tracking-wider bg-transparent text-slate-500 dark:text-white/50 border-b border-slate-200/70 dark:border-white/10">
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">No</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">Tanggal</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">Posyandu</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">NIK</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">Pelapor</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">Alamat</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">Hal Pengaduan</th>
                    <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-white/10 text-center">Data Dukung</th>
                    <th colSpan={2} className="px-6 py-2 text-center bg-slate-100 dark:bg-[#2f2f2f] text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-white/10">Keterangan</th>
                    {canEdit && <th rowSpan={2} className="px-6 py-4 text-right">Aksi</th>}
                  </tr>
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <th className="px-4 py-2 border-r border-slate-200 dark:border-white/10 text-center">TL</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={report.id} className="border-b border-slate-200/70 dark:border-white/10 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                      <td className="px-6 py-4 text-center font-medium text-[var(--dash-text)]">{idx + 1}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                ...
                <div className="md:col-span-1">
                  <label className="text-sm font-medium text-[var(--dash-text)] dark:text-slate-200 block mb-1.5">NIK (Opsional, 16 digit)</label>
                  <input
                    type="text"
                    maxLength={16}
                    value={formData.nik}
                ...
                <div className="md:col-span-2 border-t border-slate-200/70 dark:border-white/10 pt-4">
                  <DataDukungUpload files={uploadedFiles} setFiles={setUploadedFiles} bidang="SOSIAL" posyanduId={selectedPosyanduId} />
                </div>
