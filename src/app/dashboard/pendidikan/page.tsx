...
export default function PendidikanPage() {
  ...
  const fetchReports = async (pId: string, yr: number) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/dashboard/pendidikan?posyanduId=${pId}&tahun=${yr}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const res = await fetch(`/api/dashboard/pendidikan?id=${id}`, {
          method: 'DELETE'
        })
        if (res.ok) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fData.append('bidang', 'PENDIDIKAN')
    if (selectedPosyanduId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/dashboard/pendidikan', {
        method: 'POST',
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl font-bold text-[var(--dash-text)]">Laporan Pendidikan</h1>
              <p className="text-[var(--dash-text-soft)] text-sm">
                {isPosyandu ? 'Pengaduan & Aspirasi Bidang Pendidikan' : `Pengaduan & Aspirasi Bidang Pendidikan - ${selectedPosyandu}`}
              </p>
            </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={report.id} className="border-b border-slate-200/70 dark:border-white/10 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                      <td className="px-6 py-4 text-[var(--dash-text)] font-medium">{index + 1}</td>
                      ...
                      {canEdit && (
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button onClick={() => handleDelete(report.id)} className="text-rose-500 hover:text-rose-600 transition-colors">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                ...
                <div className="md:col-span-2 border-t border-slate-200/70 dark:border-white/10 pt-4">
                  <label className="text-sm font-medium text-[var(--dash-text)] dark:text-slate-200 block mb-1.5">Status Tindak Lanjut</label>
